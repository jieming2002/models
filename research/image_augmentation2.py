...
# just augmentation the low and wrong image.
...
def preprocess_each(filenames):
    ...
    for fname in filenames:
        if fnum % 10 == 0:
            print(fnum, fname)
        fnum = fnum + 1
        with tf.gfile.FastGFile(fname, 'rb') as f_image:
            value = f_image.read()
            # print('fname=', fname)
            image_data = tf.image.decode_jpeg(value, channels=3) 
            #对一个图片进行多次增广 1~9 共 9 次，加上原图，训练集总共扩大到原来的 10 倍
            augment_num = get_augment_num(fname)
            results = []
            for i in range(1, augment_num): 
                # result = preprocess_for_train(image_data, 299, 299, boxes)
                # yield (i, fname, result)
                results.append(preprocess_for_train(image_data, 299, 299, boxes))
        yield (fname, results)

def get_augment_num(filename):
    augment_num = 0
...
def get_outputname(filename, ix):
    ...
    f_dir = os.path.dirname(os.path.dirname(filename))
    # print('f_dir=', f_dir)
    ...
    # name_list = os.path.splitext(basename)
    # target_name = name_list[0] + '.jpg'
...
with tf.Session() as sess:  
    ...
    for fname, results in preprocess_each(filenames[(step-1)*step_size:step*step_size+1]):
        # break
        for ix, result in enumerate(results):
            output_filename = get_outputname(fname, ix+1)
    ...
    #             print(global_step, output_filename)
            ...
            # scipy.misc.imsave(output_filename, reimg) 
        # break
