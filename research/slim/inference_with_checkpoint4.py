...
def check_result(fpath, path_wrong, path_low, filename, y_label, top_5, top1_p):
    ...
    if(top_5[0] != y_label):
        # plt.imsave(os.path.join(path_wrong, filename), reimg)
        # plt.imsave(os.path.join(path_wrong, filename), plt.imread(fpath))
        shutil.copyfile(fpath, os.path.join(path_wrong, top_5+filename))
    elif (top1_p < 9.5):
        # plt.imsave(os.path.join(path_low, filename), plt.imread(fpath))
        ...
        if os.path.isdir(path):
            ...
            for filename in os.listdir(path):
                ...
                if not os.path.isdir(fpath):
                    image_value = open(fpath, mode='rb').read()
                    ...
                    num_images += 1
                    print(y_label + ' '+ str(num_images // 400) +' =', num_images % 400)
                    # break
        # break
